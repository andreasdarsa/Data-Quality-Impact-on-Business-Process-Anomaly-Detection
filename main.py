...
from detectors.knn_detector import KNNDetector
...
for k in [20, 30, 50, 100]:
    knn = KNNDetector(n_neighbors=k)
    knn.fit(X_train)

    scores = knn.score_samples(X_test)
    ...
    for perc in [85, 90, 95]:
        ...
        f1 = f1_score(y_test, y_pred)
        print(f"KNN k={k}, perc={perc}, F1: {f1:.2f}")

        if f1 > knn_best_f1:
            knn_best_f1 = f1
            knn_best = (k, perc, y_pred)

print(f"Best KNN eps: {knn_best[0]}, F1: {knn_best_f1:.2f}")
...
evaluate(y_test, knn_best[2], name="KNN")
...
evaluate_per_subtype(y_test, knn_best[2], sub_test, name="KNN")
